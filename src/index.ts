...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Usda FDC');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'search_foods',
    description: 'Search USDA Food Data Central for food items by name, brand, or ingredient. Supports filtering by data type (Branded, Foundation, SR Legacy, Survey FNDDS, Experimental) and brand owner. Returns FDC id, description, data type, and publication date per result.',
    inputSchema: {
      type: 'object',
      properties: {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  // list_food_groups is hardcoded reference data — no upstream call,
  // so it doesn't need a key. Anything that actually hits the FDC API
  // does. Gate the key check to those tools, not all tools.
  if (!apiKey && name !== 'list_food_groups') {
    throw new Error(
  ...
  switch (name) {
    ...
    case 'list_food_groups':
      // FDC doesn't expose a /food-groups endpoint, so we return the canonical
      // WWEIA category set directly. No upstream call needed — the previous
      // version did a "validation" call to /foods/list that 404'd when the
      // platform key was invalid, surfacing as a broken tool to agent probes.
      return {
        ...
        common_dataTypes: ['Foundation', 'SR Legacy', 'Survey (FNDDS)', 'Branded', 'Experimental'],
      };
