...
`tools/list` at `https://gateway.pipeworx.io/usda-fdc/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_foods \
  -H 'Content-Type: application/json' \
  -d '{"query":"organic whole milk"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_foods`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
